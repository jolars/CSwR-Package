...
#'
#' Tracer objects can collect, print and summarize trace information from the
#' evaluation environment of other functions during their evaluation.
#'
#' The function \code{tracer} constructs a tracer object containing a
#' \code{tracer}, a \code{get} and a \code{clear} function. A call of the
#' objects \code{tracer} function can be inserted in other functions and used to
#' collect, print and plot trace information about the internals of that
#' function during its evaluation. The objects \code{get} function can access
#' that information afterwards, and its \code{clear} function deletes all stored
#' values in the tracer object.
#'
#' A tracer object can trace time (in seconds) between \code{tracer} calls,
#' which are measured by the \code{hires_time} function from the bench package.
#' There are \code{print} and \code{summary} methods available for summarizing
#' the trace information. A call of the \code{tracer} function can be manually
#' inserted into the body of the function that is to be traced, it can be
#' inserted using \code{base::trace}, or it can be passed as an argument to any
#' function with a callback argument.
#'
#' @param objects a character vector of names of the objects that are to be
#'   traced. The objects are searched for in the calling environment of the
#'   \code{tracer} function. Objects created by the \code{expr} argument can
#'   also be traced.
#' @param Delta an integer specifying if and how often trace information is
#'   printed. \code{Delta = 0} means never, and otherwise trace information is
#'   printed every \code{Delta}-th iteration. \code{Delta = 1} is the default.
#' @param save a logical value. Determines if the trace information is stored.
#' @param time a logical value. Determines if runtime information in seconds is
#'   traced.
...
#'
#' @return
#' \code{summary} returns a data frame (of class trace) with columns containing
#' the values of the traced objects, and if time is traced an additional column,
#' \code{.time}, containing the cumulative runtime in seconds.
#' @export
#'
#' @examples
#' tr <- tracer("i", Delta = 0)
